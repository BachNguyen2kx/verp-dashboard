...
graph LR
    subgraph PEOPLE ["CON NGƯỜI (PEOPLE)"]
        P1["Thiếu hụt thợ QC chuyên môn"] --> P1_Detail["Ứ đọng khâu kiểm thử (đạt 60% tiến độ)"]
        P2["Công nhân Sơn/PU nghỉ việc nhiều"] --> P2_Detail["Trễ tiến độ xưởng sơn (chỉ đạt 74%)"]
        P1_Detail --> Cause_People
        P2_Detail --> Cause_People
    end
    
    subgraph MATERIAL ["NGUYÊN VẬT LIỆU (MATERIAL)"]
        M1["Đầu cơ gỗ quá mức / Hủy đơn hàng"] --> M1_Detail["Gỗ đỏ Nam Phi đọng 91.3 ngày"]
        M2["Mua hàng số lượng lớn (Bulk buying)"] --> M2_Detail["Vốn kẹt kho nguyên liệu chính >65%"]
        M1_Detail --> Cause_Material
        M2_Detail --> Cause_Material
    end
    
    subgraph MACHINE ["THIẾT BỊ (MACHINE)"]
        MC1["Lỗi kỹ thuật chà nhám / Rung lắc máy"] --> MC1_Detail["PX3 lỗi 1.34% dù chạy ít giờ"]
        MC2["Hệ thống thông tin rời rạc"] --> MC2_Detail["Thiếu UI giám sát OEE/Defect Rate thực tế"]
        MC1_Detail --> Cause_Machine
        MC2_Detail --> Cause_Machine
    end
    
    subgraph PROCESS ["QUY TRÌNH (PROCESS)"]
        PR1["Quy trình hành chính thủ công & cồng kềnh"] --> PR1_Detail["Chi phí SG&A quá cao (24.6% doanh thu)"]
        PR2["Sản xuất dòng hàng thử nghiệm kém hút khách"] --> PR2_Detail["Bàn ATLA tồn kho lâu (92.3 ngày)"]
...
1. **Biên lợi nhuận ròng chưa tối ưu (18.94%)**:
   * *Nguồn gốc số liệu*: Doanh thu thuần đạt **9.9M USD**, Lợi nhuận gộp đạt **3.93M USD** (Biên lợi nhuận gộp rất cao **39.75%**). Tuy nhiên, Lợi nhuận ròng sau thuế chỉ còn **1.88M USD**, dẫn đến Biên lợi nhuận ròng chỉ đạt **18.94%** (lấy từ dữ liệu kết chuyển P&L trên Dashboard).
   * *Nguyên nhân gốc rễ*: Lợi nhuận gộp mạnh bị bào mòn bởi chi phí vận hành quá lớn:
     * **Chi phí quản lý doanh nghiệp (SG&A)**: Lên tới **2.45M USD** (chiếm **24.6%** doanh thu thuần, mã nhóm `EXP_SGNA` trong tệp [chi_phi_tai_san_chi_tiet.csv](data/ke_toan/chi_phi_tai_san_chi_tiet.csv)). Điều này phản ánh các quy trình hành chính, quản lý nhân sự gián tiếp và quản lý văn phòng vẫn còn thủ công, cồng kềnh, chưa được tự động hóa.
     * **Chi phí bán hàng**: Đạt **1.18M USD** (chiếm **11.9%** doanh thu, mã nhóm `EXP_SELL`), chủ yếu do chi phí vận tải quốc tế tăng cao và các chương trình tiếp thị chưa đạt hiệu quả chuyển đổi tối ưu.
...
3. **Vốn lưu động kẹt ở kho Nguyên liệu chính (>65%) và Gỗ đỏ Nam Phi đọng vốn (91.3 ngày)**:
   * *Nguồn gốc số liệu*: Giá trị tồn kho nguyên vật liệu chính chiếm tới **65%** tổng cơ cấu tồn kho cuối kỳ (tính từ [ton_kho_theo_thang.csv](data/san_xuat/ton_kho_theo_thang.csv)). Số ngày lưu kho của gỗ đỏ Nam Phi đạt trung bình **91.34 ngày** (tính từ tệp [chi_tiet_phieu_kho.csv](data/san_xuat/chi_tiet_phieu_kho.csv)).
   * *Nguyên nhân gốc rễ*: 
...
5. **Quy trình lưu kho kéo dài đối với bàn làm việc thông minh ATLA (92.3 ngày)**:
   * *Nguồn gốc số liệu*: Bàn làm việc ATLA lưu kho trung bình **92.36 ngày** (từ tệp [ton_kho_thanh_pham_theo_thang.csv](data/kinh_doanh/ton_kho_thanh_pham_theo_thang.csv)).
   * *Nguyên nhân gốc rễ*: Đây là dòng sản phẩm thử nghiệm thế hệ mới tích hợp sạc không dây và điều khiển nâng hạ thông minh. Kích thước bàn cồng kềnh, khó tháo lắp đóng thùng dẹt, chi phí vận chuyển cao. Hơn nữa, thiết kế chưa thực sự khớp thị hiếu chuộng phong cách mộc tự nhiên của tệp khách hàng xuất khẩu truyền thống, dẫn đến tốc độ đẩy hàng đi cực kỳ chậm.
