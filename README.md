...
graph LR
    subgraph PEOPLE ["PEOPLE"]
        P1["Shortage of specialized QC staff"] --> P1_Detail["Bottleneck in final inspection (60% completion rate)"]
        P2["High worker turnover in Painting/PU"] --> P2_Detail["Painting stage delay (only 74% completion)"]
        P1_Detail --> Cause_People
        P2_Detail --> Cause_People
    end
    
    subgraph MATERIAL ["MATERIAL"]
        M1["Over-purchasing wood / Canceled orders"] --> M1_Detail["S. African Red Wood stored for 91.3 days"]
        M2["Bulk buying without JIT strategy"] --> M2_Detail["Capital locked in raw materials >65%"]
        M1_Detail --> Cause_Material
        M2_Detail --> Cause_Material
    end
    
    subgraph MACHINE ["MACHINE"]
        MC1["Sanding inaccuracies / Mechanical vibrations"] --> MC1_Detail["WS3 has 1.34% defect rate despite low run hours"]
        MC2["Fragmented information systems"] --> MC2_Detail["Lack of real-time OEE/Defect Rate dashboard"]
        MC1_Detail --> Cause_Machine
        MC2_Detail --> Cause_Machine
    end
    
    subgraph PROCESS ["PROCESS"]
        PR1["Manual & bloated administrative workflows"] --> PR1_Detail["SG&A overhead too high (24.6% of revenue)"]
        PR2["Production of niche trial furniture models"] --> PR2_Detail["ATLA Smart Desk inventory stored for 92.3 days"]
...
1. **Squeezed Net Profit Margin (18.94%)**:
   * *Data Source*: Net Revenue is **$9.9M**, Gross Profit is **$3.93M** (high Gross Margin of **39.75%**). However, Net Profit is only **$1.88M**, leading to a Net Margin of **18.94%** (as calculated in the P&L module of the dashboard).
   * *Root Cause*: High gross profits are eroded by excessive operational expenses:
     * **General & Administrative Expenses (SG&A)**: Reaches **$2.45M** (representing **24.6%** of net revenue, categorized as `EXP_SGNA` in [chi_phi_tai_san_chi_tiet.csv](data/ke_toan/chi_phi_tai_san_chi_tiet.csv)). This reflects manual administrative workflows, redundant supervisory roles, and office overheads that have not been automated.
     * **Selling Expenses**: Stands at **$1.18M** (representing **11.9%** of net revenue, categorized as `EXP_SELL`), driven by rising international logistics costs and inefficient marketing campaigns.
...
3. **Working Capital Tied Up in Raw Materials (>65%) and South African Red Wood (91.3 days)**:
   * *Data Source*: Raw materials inventory value accounts for over **65%** of the total inventory value (calculated from [ton_kho_theo_thang.csv](data/san_xuat/ton_kho_theo_thang.csv)). The average days in stock for South African Red Wood stands at **91.34 days** (calculated from [chi_tiet_phieu_kho.csv](data/san_xuat/chi_tiet_phieu_kho.csv)).
   * *Root Cause*: 
...
5. **Extended Inventory Duration for ATLA Smart Desks (92.3 days)**:
   * *Data Source*: ATLA Smart Desks remain in stock for an average of **92.36 days** (retrieved from [ton_kho_thanh_pham_theo_thang.csv](data/kinh_doanh/ton_kho_thanh_pham_theo_thang.csv)).
   * *Root Cause*: This experimental furniture line integrates wireless chargers and electronic height-adjustable modules. Its bulky shape is difficult to flat-pack, leading to high shipping volumes. Furthermore, the design has not fully aligned with traditional export clients who prefer solid rustic designs, resulting in slow sales.
